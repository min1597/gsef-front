        ...
        div.w-full
            ...
            div.w-full.flex.flex-wrap.overflow-x-auto
                template(v-if='loading.status == false && data.gender !== "" && data.sport !== ""')
                    ...
                    template(v-for='(person, index) of (data.sport == "run" ? ([ ... response[data.gender][data.sport].sort((a, b) => { return (data.sport == "total" ? a[data.sport] : a[data.sport].value) - (data.sport == "total" ? b[data.sport] : b[data.sport].value) }).filter(_d => _d.run.value !== null && _d.run.value !== 0), ... response[data.gender][data.sport].sort((a, b) => { return (data.sport == "total" ? a[data.sport] : a[data.sport].value) - (data.sport == "total" ? b[data.sport] : b[data.sport].value) }).filter(_d => _d.run.value == null || _d.run.value == 0) ]) : (data.sport == "flex" ? [ ... response[data.gender][data.sport].sort((a, b) => { return (data.sport == "total" ? a[data.sport] : a[data.sport].value) - (data.sport == "total" ? b[data.sport] : b[data.sport].value) }).reverse().filter(_d => _d.flex.value !== 0 && _d.flex.value !== null), ... response[data.gender][data.sport].sort((a, b) => { return (data.sport == "total" ? a[data.sport] : a[data.sport].value) - (data.sport == "total" ? b[data.sport] : b[data.sport].value) }).reverse().filter(_d => _d.flex.value == 0 || _d.flex.value == null) ] : response[data.gender][data.sport].sort((a, b) => { return (data.sport == "total" ? a[data.sport] : a[data.sport].value) - (data.sport == "total" ? b[data.sport] : b[data.sport].value) }).reverse() ))')
                        div.border-b.px-5.py-2.flex.justify-start.items-center(v-if="toJSON(person).indexOf(searchData.value) !== -1")
                            ...
                            template(v-else-if="data.sport == 'flex'")
                                div.w-12.text-center(v-if="person.flex.value !== 0 && person.flex.value !== null") {{ response[data.gender][data.sport].filter(_person => _person.flex.value !== 0 && _person.flex.value !== null).filter(_person => { return (data.sport == 'total' ? _person[data.sport] : _person[data.sport].value) > (data.sport == 'total' ? person[data.sport] : person[data.sport].value) }).length + 1 }}
                                div.w-12.text-center(v-else) -
                            template(v-else)
